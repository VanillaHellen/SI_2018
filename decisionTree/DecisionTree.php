    ...
    private function find_root($parent_node, $branch_name, $training_data)
    {
        if ($parent_node->value == 'Root') {
            if ($this->display_debug) {
                print_r("Node:Root  Branch:Any\n");
            }
        } else {
            if ($this->display_debug) {
                print_r("Node:" . $parent_node->value . " Branch:" . $branch_name . "\n");
            }
        }
        if ($this->display_debug) {
        ...
        if (count($p) == 1) {
            reset($p);
            if ($this->display_debug) {
                print_r("End of this branch with value:" . strtoupper(key($p)) . "!!\n\n");
            }
        ...
        if ($this->display_debug) {
            print_r("New Root attribute:" . $winning_attribute . "\n");
        }
        ...
        foreach ($p as $value => $count) {
            ...
            if ($this->display_debug) {
                print_r($winning_attribute . "->" . $value . "\n");
            }
